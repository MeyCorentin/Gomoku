...
{
    return ((pionNumberInDirection(position, direction) == 5) ? true: false);
}
...
bool MinMax::fiveSplit(std::pair<int, int> position, std::pair<int,int> direction)
{
    ...
    if (position.first + (direction.first * 5) >= (_bitboard->getRowSize()) ||
        ...
        return false;
    if (_bitboard->getBit(std::make_pair(position.first - direction.first, position.second - direction.second)) == (is_begin ? 1 : 2) &&
...
bool MinMax::fourOpen(std::pair<int, int> position, std::pair<int,int> direction)
{
    ...
    if (position.first + (direction.first * 5) >= (_bitboard->getRowSize()) ||
        ...
        position.second + (direction.second * 5) < 0)
        return false;
    if (_bitboard->getBit(std::make_pair(position.first, position.second)) == 0 &&
...
bool MinMax::fourSplit(std::pair<int, int> position, std::pair<int,int> direction)
{
    ...
    if (_bitboard->getBit(std::make_pair(position.first - direction.first, position.second - direction.second)) == (is_begin ? 1 : 2) &&
        ...
        return true;
    if (_bitboard->getBit(std::make_pair(position.first - direction.first, position.second - direction.second)) == 0 &&
